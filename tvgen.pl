...
die "tvgen.pl: cannot find libdir" if !$LIB;
...
my $conf_h = "./$opt{p}_tv.tmpl";
my %fmap = (
	    "$LIB/tv.code"    => "$opt{p}_tv.c",
	    "$LIB/tv.private" => "$opt{p}_tvpriv.h",
	    "$LIB/tv.public"  => "$opt{p}_tvpub.h",
	    $conf_h => "$opt{p}_tv.h",
	   );
if (!-e $conf_h) {
    print "cp $LIB/tv.setup $conf_h\n";
    system("cp $LIB/tv.setup $conf_h")==0 or die "cp $LIB/tv.setup $conf_h: $!";
}
...
for my $f (sort keys %fmap) {
    ...
    if ($f =~ m/tv.code$/) {
	$to->print(qq{\#include "$opt{p}_tv.h"\n});
	$to->print("#define _TV_SNEAKY_ON_\n");
	$to->print(qq{\#include "$opt{p}_tvpriv.h"\n});
    }
